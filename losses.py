...
def calc_ssim(y_true, 
    ...
    mu_true = tf.nn.depthwise_conv2d(_y_true, g_kernel, strides=[1, 1, 1, 1], padding='VALID')
    mu_pred = tf.nn.depthwise_conv2d(_y_pred, g_kernel, strides=[1, 1, 1, 1], padding='VALID')
...
def make_gaussian_pyramid(x, 
    ...
    for level in range(max_level):
        ...
        for _ in range(gaussian_iteration):
            filtered = tf.nn.depthwise_conv2d(filtered, g_kernel, strides=[1, 1, 1, 1], padding='SAME')
